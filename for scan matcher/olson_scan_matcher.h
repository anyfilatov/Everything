...
public:
  Olson_scan_matcher(double _radius, double _sigma, double _meters_per_cell) :
                     ...
                     meters_per_cell(_meters_per_cell*10.0), table(nullptr) {
    radius = meters_per_cell*3.0;
  }
  ...
  void blur_probability(DiscretePoint2D point) {
    if(table == nullptr) {
      return;
    }
    ...
    for (double i = -radius; i <= radius; i += step) {
      ...
      for (double j = -range_j; j <= range_j; j += step) {
        double prev_value = table->cell_value(DiscretePoint2D(point.x + i, point.y + j));
        double value = prev_value + normal_variation(i,j,0.0, sigma);
        //value = value > 1.0 ? 1.0 : value;
        table->update_cell(table->world_to_cell(point.x + std::floor(i), point.y + std::floor(j)), Occupancy(value, 1.0), 1.0);
      }
    }
  }
  ...
  std::vector<DiscretePoint2D> scan_to_cartezian(const RobotState& pose,
                                        const TransformedLaserScan& scan,
                                        double &x_min, double &x_max,
                                        double &y_min, double &y_max) {
    std::vector<DiscretePoint2D> result;
    x_min = 10000;
    y_min = 10000;
    x_max = -10000;
    y_max = -10000;
    ...
    for (auto point : scan.points) {
      ...
      result.push_back(DiscretePoint2D(x_world, y_world));
    }
    return result;
  }
  ...
  void generate_table(const RobotState& pose,
    ...
    std::vector<DiscretePoint2D> current_cartezian_scan =
  ...
  double esimate_scan_prob(const TransformedLaserScan& scan, const RobotState &pose, const GridMap& map){
    ...
    for (auto point : scan.points) {
         ...
         DiscretePoint2D cell_coord = table->world_to_cell(x_world, y_world);
         double point_prob = table->cell_value(cell_coord);
         result += point_prob;
       }
    result /= i;
  ...
  double process_scan(const RobotState &init_pose,
    ...
    int i = 0;
    for (double dx = -std::abs(odom_x)*2.0; dx < std::abs(odom_x)*2.0; dx += std::abs(odom_x)/20.0) {
      for (double dy = -std::abs(odom_y)*2.0; dy < std::abs(odom_y)*2.0; dy += std::abs(odom_y)/20.0) {
        for (double dt = -std::abs(odom_theta)*2.0; dt < std::abs(odom_theta)*2.0; dt += std::abs(odom_theta)/20.0) {
          ...
          double scan_probability = esimate_scan_prob(scan, RobotState(init_pose.x + dx,init_pose.y + dy,init_pose.theta + dt), map);
